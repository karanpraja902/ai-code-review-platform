    ...
    try {
      ...
      if (details.installationId && details.issueNumber && details.fullName) {
        try {
          ...
          const body = [
            '🚀 Analyze and fix this issue with **[ai-code-review](https://github.com/apps/ai-code-review)**.',
            '',
...
export const handleStopAnalysis = async (payload: {
  ...
  try {
    ...
    if (githubRepo) {
      ...
      const result = await Analysis.updateMany(
        ...
        },
        {
          $set: { 
            status: 'interrupted',
            errorLogs: 'Manually stopped via @ai-code-review stop command.'
          },
        }
      );
    ...
    if (interruptedCount > 0) {
      try {
        // Find the existing AI Code Review check run for this PR
        ...
        if (ai-code-reviewCheck) {
          await octokit.checks.update({
            ...
            output: {
              title: 'AI Code Review Platform review cancelled',
          ...
          try {
            await octokit.repos.createCommitStatus({
              ...
              target_url: `https://ai-code-review-platform.dev/github/${encodeURIComponent(repository.full_name)}/pull/${prNumber}`,
            });
          } catch (statusErr) {
...
export const PrData = async (payload: any, options?: { skipBotCheck?: boolean }) => {
  try {
    ...
    // Skip this check if triggered by @ai-code-review review command
    if (!skipBotCheck) {
    ...
    // Helper to check if a commit is authored or co-authored by a bot (e.g., AI Code Review)
    ...
    try {
      ...
      if (githubRepo && githubRepo.trackGithubPullRequests) {
       ...
       // Generate unique analysis ID for this PR analysis
        ...
        // Create/Update GitHub Check Run to reflect AI Code Review Platform review status
        let checkRunId: number | undefined;
        let usedStatusFallback = false;
        try {
          const detailsUrl = `https://ai-code-review-platform.dev/github/${encodeURIComponent(repository.full_name)}/pull/${pull_request.number}`;
          const checkRun = await octokit.checks.create({
            ...
            output: {
              title: 'AI Code Review Platform is reviewing…',
          ...
          logger.info('Created AI Code Review Platform check run', { checkRunId, prNumber: pull_request.number, repository: repository.full_name });
        } catch (checkErr) {
          logger.warn('Failed to create GitHub Check Run (AI Code Review Platform Review). Ensure app has checks:write permission.', {
            error: checkErr instanceof Error ? checkErr.message : checkErr,
            prNumber: pull_request.number,
            repository: repository.full_name
          });
        ...
        executeAnalysis(
          ...
          callbacks,
          {
            ...
            base_url: process.env.API_BASE_URL || "https://api.ai-code-review-platform.dev",
            pr_number: pull_request.number,
            pr_url: prUrl,
            pr_title: pull_request.title,
            repo_url: repoUrl,
            ...
            if (checkRunId) {
              try {
                await octokit.checks.update({
                  ...
                  conclusion: 'failure',
                  output: {
                    title: 'AI Code Review Platform review failed',
                    summary: `Analysis encountered an error for PR #${pull_request.number}.`,
                    text: result.error || 'Unknown error.'
                  }
                });
                logger.info('Updated AI Code Review Platform check run to failed', { checkRunId, prNumber: pull_request.number });
              } catch (updateErr) {
              ...
              try {
                await octokit.repos.createCommitStatus({
                  ...
                  state: 'failure',
                  context: 'AI Code Review',
                  description: 'AI Code Review Platform review failed',
                  target_url: `https://ai-code-review-platform.dev/github/${encodeURIComponent(repository.full_name)}/pull/${pull_request.number}`
                });
              } catch (statusErr) {
          ...
          if (checkRunId) {
            try {
              await octokit.checks.update({
                ...
                conclusion: 'success',
                output: {
                  title: 'AI Code Review Platform review completed',
                  summary: `AI Code Review Platform finished analyzing PR #${pull_request.number}.`,
                  text: `Posted ${finalComments.length} additional comments.`
                }
              });
              logger.info('Updated AI Code Review Platform check run to completed', { checkRunId, prNumber: pull_request.number });
            } catch (updateErr) {
            ...
            try {
              await octokit.repos.createCommitStatus({
                ...
                state: 'success',
                context: 'AI Code Review',
                description: 'AI Code Review Platform review completed',
                target_url: `https://ai-code-review-platform.dev/github/${encodeURIComponent(repository.full_name)}/pull/${pull_request.number}`
              });
            } catch (statusErr) {
