...
export class PRCommentService {
  ...
  private static STATUS_MARKER = '<!-- ai-code-review:main-comment -->';
...
// Ensure one blank line after table (after last row)
  ...
  async postComment(comment: PRComment): Promise<boolean> {
    try {
      ...
      // Summary comments: update the initial AI Code Review status comment instead of posting a new one
  ...
  private generateUserGuideFooter(): string {
    ...
    return [
      ...
      '- Comment `@ai-code-review` on any PR to start analysis manually',
  ...
  async postSkippedComment(reason: string, showTriggerHint: boolean = true): Promise<boolean> {
    try {
      ...
      if (showTriggerHint) {
        bodyParts.push('>');
        bodyParts.push('> To trigger a single review, invoke the `@ai-code-review` command.');
      }
      ...
      bodyParts.push('');
      bodyParts.push('---');
      bodyParts.push(`Follow us: [AI Code Review](https://ai-code-review-platform.dev) · [X](https://x.com/ai-code-reviewai_dev) · [LinkedIn](https://www.linkedin.com/company/ai-code-review)`);
  ...
  */
  async postDailyLimitReachedComment(): Promise<boolean> {
    return this.postSkippedComment("You've hit the daily PR analysis limit on your current plan. Consider upgrading: [Upgrade](https://ai-code-review-platform.dev/upgrade)", false);
  }
  ...
  /**
   * Update the AI Code Review status comment with the provided summary content.
