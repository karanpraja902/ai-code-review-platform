...
const aiModelSchema = new Schema<IAIModel>(
  {
    ...
    provider: { type: String, required: true, enum: ['bedrock', 'google', 'vertex'] },
    allowedPlans: [{ type: Schema.Types.ObjectId, ref: 'Subscription_Plan', index: true }],
    isActive: { type: Boolean, required: true, default: true },
    allowedModes: [{
      type: String,
      enum: ['pr_analysis', 'full_repo_analysis', 'extension_analysis'],
      default: ['pr_analysis', 'full_repo_analysis', 'extension_analysis'],
    }],
  },
  { timestamps: true }
);
